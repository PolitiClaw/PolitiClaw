...
- [`politiclaw_configure`](../reference/generated/tools/politiclaw_configure) — the agent-facing tool. Pass any subset of keys; entries you do not provide are skipped. Use this for "save this one key" calls without re-running the full setup flow.
...
2. Add `googleCivic` if you want ballot tools (`politiclaw_get_my_ballot`, `politiclaw_election_brief`, `politiclaw_election_brief`).
